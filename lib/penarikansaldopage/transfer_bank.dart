...
class _TransferBankState extends State<TransferBank> {
  ...
  Future<bool?> penguranganSaldo(int id, int saldo) async {
    try {
      ...
      var response =
          await http.put(Uri.parse(baseUrl! + '/api/minsaldo/' + id.toString()),
              headers: {
                'X-API-Key': "myfin",
  ...
  Future<bool?> penarikanSaldo(int id, int jumlahPenarikan, String bank,
      String norek, String namaPemilik) async {
    try {
      SharedPreferences server = await SharedPreferences.getInstance();
      String? baseUrl = server.getString('server');
      ...
      final msg = jsonEncode({
        "id": id,
        "jml_penarikan": jumlahPenarikan,
        "bank": bank,
        "norek": norek,
        "nama_pemilik": namaPemilik
      });
      var response =
          await http.post(Uri.parse(baseUrl! + '/api/penarikansaldo'),
              headers: {
                'X-API-Key': "myfin",
                'Accept': "application/json",
              },
          ...
          children: [
            ...
            Container(
              ...
              children: [
                ...
                ConstrainedBox(
                  constraints: BoxConstraints.tightFor(width: 128, height: 50),
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      var datapenarikan = await penarikanSaldo(
                          id!,
                          int.parse(jumlahPenarikan),
                          bank!,
                          norek,
                          namaPemilik);

                      if (datapenarikan == true) {
                        var datapengurangan = await penguranganSaldo(
                            id!, int.parse(nominalSaldo));
                        showprogess(context);
                      ...
                      setState(() {
                        _penarikansaldoNasabah =
                            datapenarikan as PenarikanSaldoModel;
                      });
                    },
                  ),
                ),
