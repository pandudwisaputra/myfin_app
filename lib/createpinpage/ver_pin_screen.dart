...
class Verpinscreen extends StatefulWidget {
  const Verpinscreen({Key? key}) : super(key: key);

  @override
  State<Verpinscreen> createState() => _VerpinscreenState();

  static Route route() {
    return MaterialPageRoute<void>(builder: (_) => const Verpinscreen());
  }
}

class _VerpinscreenState extends State<Verpinscreen> {
  TextEditingController pinController = TextEditingController();
  late CreatePinModel _createPin;
  ...
  Future<bool?> createpinData(String pin) async {
    try {
      ...
      var response =
          await http.put(Uri.parse(baseUrl! + '/api/pinnasabah/' + stringId),
              headers: {
                'X-API-Key': "myfin",
  ...
  validatePin() async {
    ...
    if (pin.length == 6) {
      ...
      String? pinlama = prefs.getString('pinkonfirmasi');
        ...
        if (data == true) {
          ...
          showDialog(
              ...
              builder: (context) => const AlertDialog(
                    title: const Text("Buat Pin Gagal"),
                  ));
        }
      } else {
        showDialog(
            context: context,
            builder: (context) => const AlertDialog(
                  title: const Text("Konfirmasi Pin Tidak Sama"),
                ));
      }
      // bool? data = await createpinData(pin);
      // if(data==true){
