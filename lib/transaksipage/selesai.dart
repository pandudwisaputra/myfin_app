...
class _SelesaiState extends State<Selesai> {
  ...
  Future<bool?> gettransaksiDitolak(int id) async {
    try {
      ...
      var response = await http.get(
        Uri.parse(baseUrl! + '/api/transaksi/$stringId/ditolak'),
        headers: {
          'X-API-Key': "myfin",
  ...
  Future<bool?> gettransaksiCatatan(int id) async {
    try {
      ...
      var response = await http.get(
        Uri.parse(baseUrl! + '/api/transaksi/$stringId/catatan'),
        headers: {
          'X-API-Key': "myfin",
          'Accept': "application/json",
        },
  ...
  Future<bool?> gettransaksiDiterima(int id) async {
    try {
      ...
      var response = await http.get(
        Uri.parse(baseUrl! + '/api/transaksi/$stringId/diterima'),
        headers: {
          'X-API-Key': "myfin",
          'Accept': "application/json",
        },
  ...
  Widget build(BuildContext context) {
    ...
    return displayTransaksiStatus == 0
        ? Center(
            child: Lottie.network(
                'https://assets4.lottiefiles.com/packages/lf20_mk6o3c37.json',
                width: 200,
                height: 200),
          )
        : idTransaksi == null
            ? Center(
                child: Lottie.network(
                    'https://assets2.lottiefiles.com/datafiles/AX0rqrGV5ahKpWr/data.json',
                    width: 400,
                    height: 400),
              )
            : RefreshWidget(
