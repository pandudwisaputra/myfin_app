...
class KonfirmasiPinBaru extends StatefulWidget {
  const KonfirmasiPinBaru({Key? key}) : super(key: key);

  @override
  State<KonfirmasiPinBaru> createState() => _KonfirmasiPinBaruState();

  static Route route() {
    return MaterialPageRoute<void>(builder: (_) => const KonfirmasiPinBaru());
  }
}

class _KonfirmasiPinBaruState extends State<KonfirmasiPinBaru> {
  ...
  Future<bool?> createupdatePin(String pin) async {
    try {
  ...
  validatePin() async {
    ...
    if (pin.length == 6) {
      ...
      String? pinlama = prefs.getString('pinLama');
      if (pin == pinlama) {
        ...
        if (data == true) {
          ...
          showDialog(
              ...
              builder: (context) => const AlertDialog(
                    title: const Text("Update Pin Gagal"),
                  ));
        }
      } else {
        showDialog(
