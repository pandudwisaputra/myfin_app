...
class _DatadiriState extends State<Datadiri> {
  ...
  Future<bool?> checkNik(String nik) async {
    try {
      ...
      var response = await http.post(Uri.parse(baseUrl! + '/api/nikcheck'),
          headers: {
            'X-API-Key': "myfin",
  ...
  Future<bool?> updateDataDiri(
    ...
    try {
      ...
      var response = await http.put(
          Uri.parse(baseUrl! + '/api/datanasabah/' + id.toString()),
          headers: {
            'X-API-Key': "myfin",
            'Accept': "application/json",
          },
  ...
  Future<bool?> cekNik(int nik) async {
    try {
      ...
      String? baseUrl = server.getString('server');
      final msg = jsonEncode({"nik": nik});
      var response = await http.post(Uri.parse(baseUrl! + '/api/getnik/'),
          headers: {
            'X-API-Key': "myfin",
            'Accept': "application/json",
          },
      ...
      if (response.statusCode == 200) {
        ...
        SharedPreferences prefstglLahir = await SharedPreferences.getInstance();
  ...
  Future uploadImageToFirebase1(BuildContext context) async {
    String fileName = basename(_croppedImage!.path);
    ...
    uploadTask = ref.putFile(io.File(_croppedImage!.path), metadata);
    ...
    await uploadTask.whenComplete(() async {
      ...
      SharedPreferences prefsurlktp = await SharedPreferences.getInstance();
      await prefsurlktp.setString('url_ktp', urlktp);
      print('Iki Link E Foto KTP  $urlktp');
    });
  }

  Future getImageSource1(ImageSource source) async {
            ...
            child: Wrap(
              children: <Widget>[
                ListTile(
                    ...
                    onTap: () {
                      getImageSource1(ImageSource.gallery);
                ...
                ListTile(
                  ...
                  onTap: () {
                    getImageSource1(ImageSource.camera);
                    Navigator.of(context).pop();
                  },
                ),
              ],
    ...
    return Scaffold(
        ...
        body: SingleChildScrollView(
          child: Container(
            ...
            child: Column(
              children: [
                CustomWidgets.textField('NIK',
                    isNumber: true,
                    textController: nikController,
                    length: 16,
                    cornerRadius: 10.0),
                Row(
                          ...
                          if (datadiri == true) {
                            ...
                            showDialog(
                                ...
                                builder: (context) => const AlertDialog(
                                      title:
                                          const Text("Berhasil Mengambil Data"),
                                    ));
                          } else {
                            ...
                            showDialog(
                                context: context,
                                builder: (context) => const AlertDialog(
                                      title: const Text("Nik Tidak Terdaftar"),
                                    ));
                          }
                        } else {
  ...
  Widget buildButton(BuildContext context) {
    return RawMaterialButton(
      onPressed: () async {
        ...
        bool? uploaddata = await updateDataDiri(id!, nik!.toString(), nama!,
            alamat!, noHp, lahir!, kota!, ktp!, swa!);
        if (nikController.text == "") {
          showDialog(
              context: context,
              builder: (context) => const AlertDialog(
                    title: const Text("Pastikan Semua Terisi Benar"),
                  ));
        } else {
          if (nama == " ") {
            showDialog(
  ...
  Future uploadImageToFirebase2(BuildContext context) async {
    String fileName = basename(_croppedFoto!.path);
    firebase_storage.Reference ref = firebase_storage.FirebaseStorage.instance
        .ref()
        .child('SwafotoKTP')
        .child('/$fileName');
    ...
    firebase_storage.UploadTask uploadTask;
    //late StorageUploadTask uploadTask = firebaseStorageRef.putFile(_imageFile);
    uploadTask = ref.putFile(io.File(_croppedFoto!.path), metadata);
    firebase_storage.UploadTask task = await Future.value(uploadTask);
    Future.value(uploadTask)
        .then((value) => {print("Upload file path ${value.ref.fullPath}")})
        .onError((error, stackTrace) =>
    ...
    await uploadTask.whenComplete(() async {
      ...
      SharedPreferences prefsurlswa = await SharedPreferences.getInstance();
  ...
  Widget foto() {
    if (_croppedFoto != null) {
      final path = _croppedFoto!.path;
      return ConstrainedBox(
        constraints: const BoxConstraints(
          maxWidth: 400,
          maxHeight: 200,
