import 'dart:convert';
import 'dart:math';
import 'package:Myfin/homepage/slider_dashboard.dart';
import 'package:Myfin/listrikpage/token_listrik_page.dart';
import 'package:Myfin/pulsadatapage/pulsa_paketdata.dart';
...
class _DashboardState extends State<Dashboard> {
  ...
  Future<bool?> gettransaksiPending(int id) async {
    try {
      ...
      var response = await http.get(
        Uri.parse(baseUrl! + '/api/transaksi/$stringId/pending'),
        headers: {
          'X-API-Key': "myfin",
  ...
  Future<bool?> getsaldoData(int id) async {
    try {
      ...
      var response = await http.get(
        Uri.parse(baseUrl! + '/api/getsaldo/' + stringId),
        headers: {
          'X-API-Key': "myfin",
          'Accept': "application/json",
        },
  ...
  Future<bool?> getProfile(int id) async {
    try {
      ...
      var response = await http.get(
        Uri.parse(baseUrl! + '/api/profile/' + stringId),
        headers: {
          'X-API-Key': "myfin",
          'Accept': "application/json",
        },
  ...
  Widget build(BuildContext context) {
    ...
    if (ava == "0") {
      avaToDisplay =
          'https://firebasestorage.googleapis.com/v0/b/myfin-ktp.appspot.com/o/KTP%2Fscaled_image_picker7138079792496807795.jpg?alt=media&token=8de953e4-3a8f-4cb5-bb9a-8dcbdb56ecd0';
    } else {
      ...
      body: saldo == null
          ? Center(
              child: Lottie.network(
                  'https://assets4.lottiefiles.com/packages/lf20_b88nh30c.json',
                  width: 150,
                  height: 150),
            )
          : ava == null
              ...
              : RefreshWidget(
                  ...
                  child: Container(
                    padding:
                        const EdgeInsets.only(top: 28, left: 27, right: 27),
                            ...
                            children: [
                              ...
                              ConstrainedBox(
                                ...
                                child: ElevatedButton(
                                  onPressed: () async {
                                    SharedPreferences idNasabah =
                                        await SharedPreferences.getInstance();
                                    int? id = idNasabah.getInt('idNasabah');
                                    bool? cekStatusAjuan =
                                        await gettransaksiPending(id!);
                                    if (nik == "0") {
                                      mustIsiData(context);
                                    } else {
